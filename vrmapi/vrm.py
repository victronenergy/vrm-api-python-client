...
class VRM_API:
	...
	def __init__(self, access_token=None, user_id = None, demo=False):
		"""
		...
		if demo: # Login as demo else with credentials
			...
			if access_token and user_id:
				...
				raise Exception('No access token or user id proviced')

			logger.debug('Initializing API with accessToken %s '%(self._auth_token))
	...
	def get_user_sites(self, extended=False):
		"""
		...
		request_url = self.USER_SITE_ENDPOINT.format(user_id=self.user_id)

		if not extended:
			sites = self._send_query_request(request_url)
		else:
			sites = self._send_query_request(request_url, data_dict={'extended': '1'})
		logger.debug("got sites for user %s %s"%(self.user_id,sites))
		return sites

	def get_user_sites_reporting(self):
		"""
		Download list of sites for logged in user
		"""
		if not self._is_initialized():
		...
		if 'records' in site:
			...
			logger.debug("got site for user with reporting enabled %s %s"%(self.user_id,site))
			return site
		return {}
	...
	def get_graph_widgets(self, inst_id, measurement_codes, instance=None, start=None, end=None):
	   """
	...
	def _send_query_request(self, url, data_dict={}):
		...
		"""
		response = None
		headers = {'X-Authorization':"Token %s"%self._auth_token}
		if self.demo:
